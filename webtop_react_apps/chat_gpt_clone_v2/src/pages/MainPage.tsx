import { useState, useEffect, useRef } from 'react';
import { Box, Stack } from '@mui/material';
import ChatMessage from '../types/ChatMessage';
import ChatHistory from '../components/ChatHistory';
import ChatInputArea from '../components/ChatInputArea';
import ModelSelector from '../components/ModelSelector';
import useAppDataStore from '../state/AppState';
import TextHero from '../components/typography/TextHero';

// const API_ENDPOINT = "//192.168.50.10:6767/ollama/generateRemote"
const API_ENDPOINT = "//192.168.50.10:10239/ollama-generate"


type ApiResponse = {
    response: string;
    context?: any;
};


export default function OllamaChat() {

    const [prompt, setPrompt] = useState('');
    const [chatHistory, setChatHistory] = useState<ChatMessage[]>([]);
    const [loading, setLoading] = useState(false);
    const [context, setContext] = useState<any | null>(null);

    const chatHistoryRef = useRef<HTMLDivElement>(null);
    const chatInputRef = useRef<HTMLTextAreaElement>(null);


    const { selectedModel, models } = useAppDataStore()
    const modelsAvailable = Boolean(selectedModel) && Boolean(models)

    const isLoading = loading && modelsAvailable

    useEffect(() => {
        const container = chatHistoryRef.current;
        if (container) {
            setTimeout(() => {
                container.scrollTo({
                    top: container.scrollHeight,
                    behavior: 'smooth',
                });
            }, 100);
        }
    }, [chatHistory]);

    useEffect(() => {
        if (!isLoading) chatInputRef.current?.focus();
    }, [loading]);


    const modelName = selectedModel.name.replace(':latest', '')


    const sendPrompt = async () => {
        const currentPrompt = prompt.trim();
        if (!currentPrompt || isLoading) return;

        setChatHistory(prev => [...prev, { type: 'user', text: currentPrompt }]);
        setPrompt('');
        setLoading(true);

        const requestBody = { model: modelName, prompt: currentPrompt, ...(context && { context }) };

        try {
            const res = await fetch(API_ENDPOINT, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(requestBody),
            });

            if (!res.ok) {
                const errorText = await res.text();
                throw new Error(`API Error (${res.status}): ${errorText || 'Failed to fetch response'}`);
            }

            const data: ApiResponse = await res.json();
            setChatHistory(prev => [...prev, { type: 'ai', text: data.response || 'AI returned an empty response.' }]);
            setContext(data.context || null);
        } catch (error) {
            setChatHistory(prev => [
                ...prev,
                { type: 'error', text: 'Error: ' + (error instanceof Error ? error.message : String(error)) },
            ]);
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => {
        setContext(null)
    }, [selectedModel])

    const contactsWidth = 100 / 6;
    const contactsWidthCss = `${contactsWidth}%`
    const chatWidthCss = `${100 - contactsWidth}%`

    return (
        <Stack
            direction={'row'}
            padding={2}
            gap={2}
            height={'100dvh'}
            marginX={'auto'}
        >

            <Stack direction={'column'} width={contactsWidthCss}>
                <ModelSelector />
            </Stack>

            <Stack direction={'column'} width={chatWidthCss}>
                {/* <TextHero>{modelName}</TextHero> */}
                <ChatHistory messages={chatHistory} loading={isLoading} ref={chatHistoryRef} />
                <ChatInputArea ref={chatInputRef} value={prompt} disabled={isLoading} onSend={sendPrompt} onChange={(value) => setPrompt(value)} />
            </Stack>



        </Stack>

    );
}
